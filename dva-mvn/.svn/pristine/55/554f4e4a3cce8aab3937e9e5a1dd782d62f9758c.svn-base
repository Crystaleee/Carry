 ...
 * @version 0.0.1
 */
function BasicLine(workSpace,data,attr,measure)
{   
	var svg = workSpace[0];
    ...
    svg.selectAll("path")	//选择<svg>中所有的<path>
    	...
    	.enter()			//选择enter部分
  	    ...
  	    .attr("d", function(d){
  		    return linePath(d.line);		//返回直线生成器得到的路径
  	    })	
  	    .attr("fill","none")
    ...
    console.log("size!!!")
    console.log(d3.selectAll(".linetooltip").size());
 ...
 * @version 0.0.1
 */
function StackLine(workSpace,data,attr,measure) 
{
	console.log(attr);
    ...
    for(i = 0; i < measure.length; i++)
    {
        ...
        for( j=0; j < data.length; j++)
        {
            ...
            currLine.push([ data[j][attr[0]] , total[j] ]); 
        }
        lines.push({name:measure[i] , line:currLine});
    }
